...
- [x] **R4** `[BE]` Cross-article claim dedup in `extraction.py`'s `process_and_store_claims` had no topic scoping — its `existing_match` vector query searched the entire `claim` table regardless of query/topic, while `run_claim_clustering` (clustering.py) was deliberately scoped by `search.query`. Fixed by joining through `evidence → article → search` and filtering `LOWER(s.query) = LOWER($query)`, matching clustering's existing pattern exactly — a falsy/empty `query` falls back to the old global (unscoped) search, mirroring `run_claim_clustering`'s `query=None` escape hatch used by the `/debug/rerun-*` admin tools. New tests in `tests/extraction/test_cross_article_scoping.py` assert on the actual SQL sent to `prisma.query_raw` (the JOIN clause and the query parameter) for both the scoped and fallback-to-global paths, plus a full happy-path test confirming a new claim is created and evidence attached correctly.

## 🟡 Phase 2 — Data-flow & contract fixes

- [x] **R5** `[FE]` `IntelligenceLayer.tsx`'s render-gate (`intel.metrics.canonicalClaims === 0 && !loading`) ignored the `status` field (`pending`/`processing`/`complete`/`failed`) that `get_search_intelligence()` was specifically built to return (per F4 above) — so the whole Claim Intelligence section rendered nothing at all, not even a loading state, for the first 10–30s of every normal search while extraction was still running. Fixed by branching on `intel.status` first: `pending`/`processing` → a "still processing" loading state (same visual style as the initial-load spinner), `failed` → a dedicated error state, `complete` + 0 claims → the original empty-state null (now genuinely means "done, found nothing"). `tsc --noEmit` passes; `eslint` on this file shows only pre-existing `any`-type issues, none introduced by this change.
- [x] **R6** `[BE]`/`[FE]` Narrative-anomaly badge (⚠️ ANOMALY) flagged a trivial 1-notch shift from a registered source as prominently as a full LEFT↔RIGHT flip — quick fix applied: `dashboard/[id]/page.tsx`'s trigger raised from `deviationScore > 0` to `deviationScore >= 2` (the max possible value, since `deviationScore` only ranges 0–2), restricting the badge to genuinely extreme deviations. **Not fixed by this change** (documented inline as a known, separate gap): the badge still structurally can't fire for any source outside the ~40-domain `SOURCE_BIAS_REGISTRY` at all, since an unregistered source's `source_bias="UNKNOWN"` forces `deviation_score` to `0.0` regardless of actual content. A real fix needs a per-source historical baseline instead of a static registry — bigger, separate effort, not attempted here.
- [x] **R7** `[BE]` `validate_articles()`'s geographic-diversity loop defaulted any source domain outside its ~45-domain regional lists to `"United States"` instead of `"Unknown"` — silently collapsing genuinely diverse international sources into a fake single-country bucket. Fixed to default to `"Unknown"`. New test: `tests/test_validation.py::test_unregistered_source_domains_are_geographically_unknown_not_us`.
