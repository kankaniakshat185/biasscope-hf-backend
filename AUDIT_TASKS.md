...
- [ ] **R2** `[BE]` `GET /results/{search_id}` and `GET /results/{search_id}/intelligence` have no authorization check at all — every other router (`history.py`, `subscriptions.py`, `debug.py`) enforces session ownership; this one doesn't. Anyone with a `search_id` (shared link, browser history, a log line) can read another user's full report — articles, AI narrative, left/right echo-chamber summaries, entity sentiment — logged in or not. Needs a product decision first: owner-only (add `get_optional_user_id` + `search_record.userId in (None, user_id)` check, matching `history.py`'s pattern) vs. intentionally shareable-by-link. Either way, make the choice explicit rather than accidental.
